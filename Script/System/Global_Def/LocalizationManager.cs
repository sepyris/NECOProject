...
public class LocalizationManager : MonoBehaviour
{
    ...
    void Awake()
    {
        if (Instance == null)
        {
            ...
            LoadLocalizedText(csvFileName);

            // 폰트 자동 로드 시도 (Resources 우선, Editor에서는 Assets/Font 스캔)
    ...
    /// <summary>
    /// CSV 파일을 읽고 데이터를 Dictionary에 저장
    /// </summary>
    private void LoadLocalizedText(string fileName)
    {
        ...
        if (csvFile == null)
        {
            Debug.LogError(string.Format(Def_UI.LOCALIZATION_CSV_NOT_FOUND, fileName));
            return;
        }
        ...
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            if (values.Length > 0 && !string.IsNullOrEmpty(values[0]))
            {
                ...
                for (int j = 1; j < headers.Length && j < values.Length; j++)
                {
                    ...
                    string text = values[j].Trim().Replace("\"", "");
...
#if UNITY_EDITOR
        // Editor 플레이 시 Assets/Font 폴더에서 자동으로 찾아 할당 (런타임 빌드에서는 동작하지 않음)
    ...
    private Font TryLoadFont(string resourceName)
    {
        ...
        try
        {
            ...
            if (f == null)
            {
                // 경고는 과다하면 시끄러우므로 로그 디버그 수준으로만 남김
...
#if UNITY_EDITOR
    // Editor 전용: Assets/Font 폴더에서 폰트 에셋을 찾아 할당(플레이 중 편의성)
    ...
    private Font LoadFontAssetFromFolder(string folder, string name)
    {
        ...
        // 다음으로 폴더 내 모든 Font를 검색해 이름 포함 여부로 매칭
